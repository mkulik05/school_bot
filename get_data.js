...
let res = async (creds, last_holidays_day, date_now, quarter_ind) => {
  let all_dates = []
  let struct = [];
  ...
  let addDays = (date, days) => {
    logger.info("addDays called")
    var result = new Date(date);
    result.setDate(result.getDate() + days);
    logger.debug("start date ", date, "days =", days, "result -", result)
    return result;
  };
  monday = addDays(date_now, 1 - date_now.getDay());
  logger.debug("monday =", monday)
  while (true) {
    let date = format_date(monday);
    //console.log(date)
    logger.info("get page html")
    ...
    for (let i = 0; i < tables.length - 1; i++) {
      ...
      if (addDays(monday, i) > last_holidays_day) {
        ...
        logger.info(lessons.length-1, "lessons")
        for (let l = 1; l < lessons.length; l++) {
          logger.info("lesson", l)
          let les = lessons[l].textContent.replace(/\s/g, "").slice(2);
          logger.debug("lesson", les)
          if (les != "") {
            let hw = "";
            try {
              ...
              //console.log(e)
              logger.error("error in getting hw(maybe there is no hw)", e)
            }
            let mark = table.getElementsByClassName("mark_box ")[l - 1].textContent.replace(/\s/g, "");
            logger.debug("mark -", mark)
            while (les.indexOf(".") >= 0) {
              logger.debug("replace all . in lesson name")
              les = les.replace(".", " ");
            }
            if (Object.keys(unstruct).includes(les)) {
              logger.info("lesson name is already in use, add space")
              les += " ";
            }
            unstruct[les] = {};
            unstruct[les]["hw"] = hw;
            unstruct[les]["mark"] = mark == "" ? 0 : parseInt(mark);
            logger.debug("structure -", JSON.stringify(unstruct[les], null, 2))
          }
          if (Object.keys(unstruct).length > 0) {
            logger.info("unstruct is not empty, add date label")
        ...
        if (Object.keys(unstruct).length > 0) {
          logger.info("unstruct is not empty, add it to array")
          struct.push(unstruct); 
          all_dates.push(fcurr_date)
        }
      }
      curr_date = addDays(curr_date, 1);
      logger.info("change date to", curr_date)
    }
    if (monday <= last_holidays_day) {
      logger.info("monday <= last_holidays_day, break from while")
      break;
    }
    monday = addDays(monday, -7);
    logger.info("change week to", monday)
  }
  session.logout(id);
